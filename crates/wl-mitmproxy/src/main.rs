...
impl XdgToplevelHandler for XdgToplevelProxy {
    fn handle_set_app_id(&mut self, slf: &Rc<XdgToplevel>, _app_id: &str) {
        slf.send_set_app_id(&self.app_id);
    }
}
...
fn run() -> Result<ExitCode, Box<dyn std::error::Error>> {
    ...
    if cli.command.is_empty() {
        run_daemon_mode(&cli, &host, &proxy_runtime_dir, shutdown_signal)?;
...
fn spawn_daemon_process(
    ...
    command.arg("--app-id").arg(&cli.app_id);
...
fn run_child_mode(
    ...
    let server_thread = thread::spawn(move || serve(listener, &target_display, &app_id, Some(server_shutdown)));
    ...
    loop {
        ...
        match listener.accept() {
            Ok((stream, _)) => {
                ...
                thread::spawn(move || {
                    if let Err(err) = run_client_session(stream, &target_display, &app_id) {
